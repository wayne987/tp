...
public class CalorieCommandParser implements Parser<CalorieCommand> {
    ...
    public CalorieCommand parse(String args) throws ParseException {
        ArgumentMultimap argMultimap =
            ArgumentTokenizer.tokenize(args, PREFIX_CALORIE_TYPE, PREFIX_EXERCISE, PREFIX_FOOD,
                    PREFIX_CALORIE_COUNT);

        String type = ParserUtil.parseCalorieType(argMultimap.getValue(PREFIX_CALORIE_TYPE).get());
        Calorie calorie;
        if (type.equals("in")) {
            if (!arePrefixesPresent(argMultimap, PREFIX_CALORIE_TYPE, PREFIX_FOOD, PREFIX_CALORIE_COUNT)
            ...
            calorie = new Input(food, calorieCount);
        } else {
            if (!arePrefixesPresent(argMultimap, PREFIX_CALORIE_TYPE, PREFIX_EXERCISE,
                    PREFIX_CALORIE_COUNT)
                    || !argMultimap.getPreamble().isEmpty()) {
                throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, AddCommand.MESSAGE_USAGE));
            }
            ...
            calorie = new Output(exercise, calorieCount);
        }
