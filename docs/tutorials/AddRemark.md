...
/**
 * Changes the remark of an existing day in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the day identified "
            + "by the index number used in the last day listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the day in the filtered day list to edit the remark
     * @param remark of the day to be updated to
     */
...
Create a new `Remark` in `seedu.address.model.day`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each day.

Simply add the following to [`seedu.address.ui.DayCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
``` java
public PersonCard(Person day, int displayedIndex) {
    //...
    remark.setText(day.getRemark().value);
}
```
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person dayToEdit = lastShownList.get(index.getZeroBased());
        Person editedDay = new Person(dayToEdit.getName(), dayToEdit.getPhone(), dayToEdit.getEmail(),
                dayToEdit.getAddress(), remark, dayToEdit.getTags());

        model.setPerson(dayToEdit, editedDay);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedDay));
    }
     ...
     * {@code dayToEdit}.
     */
    private String generateSuccessMessage(Person dayToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, dayToEdit);
    }
```
